...
<div class="wrap" style="padding-block:1.6rem">
  ...
  <div class="row-between" style="margin-bottom:1.1rem">
    ...
    <form method="get" action="<?= $basePath ?>/admin/users" class="row" style="gap:.4rem">
      ...
      <table class="data">
        ...
        <?php foreach ($users as $u): ?>
          <tr>
            ...
            <td>
              <form method="post" action="<?= $basePath ?>/admin/users/<?= (int) $u->id() ?>" class="row" style="gap:.3rem">
            ...
            <td>
              <div class="row" style="gap:.25rem">
                <form method="post" action="<?= $basePath ?>/admin/users/<?= (int) $u->id() ?>" style="margin:0">
                  <input type="hidden" name="_csrf" value="<?= $e($csrfToken) ?>">
    ...
    <div class="row" style="margin-top:1rem">
      <?php for ($p = 1; $p <= min($pages, 12); $p++): ?>
        <a class="btn btn-sm <?= $p === $page ? '' : 'btn-secondary' ?>"
           href="<?= $basePath ?>/admin/users?page=<?= $p ?><?= $search !== '' ? '&q=' . urlencode($search) : '' ?>"><?= $p ?></a>
      <?php endfor; ?>
    </div>
  <?php endif; ?>
</div>
